...
const SINGLE_T_R_ALARM_CONFIG = 't_r_alarm_config'; // 项目配置表
const SINGLE_T_R_USER = 't_r_user'; // 用户表
const SINGLE_T_R_PROJECT_MEMBER = 't_o_project_member'; // 项目成员表
const SINGLE_T_R_NEW_USER_SUMMARY = 't_r_new_user_summary'; // 新用户统计表
const SINGLE_T_R_ALARM_LOG = 't_r_alarm_log'; // 报警记录表

// 需要分表
const MUILT_T_O_MONITOR = 't_r_monitor'; //  异常数据表, 按项目按月分表, 只有最基础的错误信息, ext字段需要到详情表中单独获取, 命名规则: t_o_monitor_项目id_YYYYMM
const MUILT_T_O_MONITOR_EXT = 't_r_monitor_ext'; // 异常数据的ext信息, 按项目按月分表, 命名规则: t_o_monitor_ext_项目id_YYYYMM
const MUILT_T_O_UV_RECORD = 't_r_uv_record'; // uv记录表, 按项目&月分表,最小统计粒度为小时 命名规则: t_o_uv_record_项目id_YYYYMM
...
const MUILT_T_O_SYSTEM_COLLECTION = 't_r_system_collection'; // 设备记录表, 按项目分表, 最小统计粒度为月, 命名规则: t_o_device_info_项目id
const MUILT_T_O_USER_FIRST_LOGIN_AT = 't_r_user_first_login_at'; // 用户首次登陆表, 用于统计新课, 按项目分表, 命名规则: t_o_customer_first_login_at_项目id
...
class GenerateSQL extends CommandsBase {
  static get signature() {
    return `
       Utils:GenerateSQL
       {projectIdList:项目id列表,逗号分割}
       {startAtYm:建表日期开始时间(包括该时间),${COMMAND_ARGUMENT_BY_MONTH}格式}
       {finishAtYm:建表日期结束时间(包括该时间),${COMMAND_ARGUMENT_BY_MONTH}格式}
       `;
  }
...
`;
    ...
    let finishAtMoment = moment(finishAtYm, COMMAND_ARGUMENT_BY_MONTH);
    let sqlContent = '-- --------分表部分---------';
    for (let projectId of projectIdList) {
      for (
        let currentAtMoment = moment(startAtYm, COMMAND_ARGUMENT_BY_MONTH);
        // 生成时间范围之内的数据表
