...
const SINGLE_T_R_PROJECT = 't_r_project';
const SINGLE_T_R_BEHAVIOR_DISTRIBUTION = 't_r_behavior_distribution';
const SINGLE_T_R_DURATION_DISTRIBUTION = 't_r_duration_distribution';
const SINGLE_T_R_HTTP_ERROR_DISTRIBUTION = 't_r_http_error_distribution';
const SINGLE_T_R_PAGE_VIEW = 't_r_page_view';
const SINGLE_T_R_SYSTEM_BROWSER = 't_r_system_browser';
const SINGLE_T_R_SYSTEM_DEVICE = 't_r_system_device';
const SINGLE_T_R_SYSTEM_OS = 't_r_system_os';
const SINGLE_T_R_SYSTEM_RUNTIME_VERSION = 't_r_system_runtime_version';
const SINGLE_T_R_UNIQUE_VIEW = 't_r_unique_view';
const SINGLE_T_R_ALARM_CONFIG = 't_r_alarm_config';
const SINGLE_T_R_USER = 't_r_user';
const SINGLE_T_R_PROJECT_MEMBER = 't_o_project_member';
const SINGLE_T_R_NEW_USER_SUMMARY = 't_r_new_user_summary';
const SINGLE_T_R_ALARM_LOG = 't_r_alarm_log';
const MUILT_T_O_MONITOR = 't_r_monitor';
const MUILT_T_O_MONITOR_EXT = 't_r_monitor_ext';
const MUILT_T_O_UV_RECORD = 't_r_uv_record';
const MUILT_T_R_CITY_DISTRIBUTION = 't_r_city_distribution';
const MUILT_T_R_PERFORMANCE = 't_r_performance';
const MUILT_T_O_SYSTEM_COLLECTION = 't_r_system_collection';
const MUILT_T_O_USER_FIRST_LOGIN_AT = 't_r_user_first_login_at';
const MUILT_T_R_ERROR_SUMMARY = 't_r_error_summary';
let TABLE_TEMPLATE = {};
TABLE_TEMPLATE[SINGLE_T_R_PROJECT] = `(
...
`;
TABLE_TEMPLATE[SINGLE_T_R_ALARM_CONFIG] = `(
...
`;
TABLE_TEMPLATE[SINGLE_T_R_USER] = `(
  \`id\` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '记录id',
...
`;
TABLE_TEMPLATE[SINGLE_T_R_PROJECT_MEMBER] = `(
    \`id\` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
...
`;
TABLE_TEMPLATE[SINGLE_T_R_NEW_USER_SUMMARY] = `(
  \`id\` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '记录id',
  \`project_id\` bigint(20) NOT NULL DEFAULT '0' COMMENT '项目id',
...
class GenerateSQL extends commandsBase_1.default {
    static get signature() {
        return `
       Utils:GenerateSQL
       {projectIdList:项目id列表,逗号分割}
       {startAtYm:建表日期开始时间(包括该时间),${config_1.COMMAND_ARGUMENT_BY_MONTH}格式}
       {finishAtYm:建表日期结束时间(包括该时间),${config_1.COMMAND_ARGUMENT_BY_MONTH}格式}
       `;
    }
    ...
    execute(args, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (_.isEmpty(projectIdList)) {
                this.log(`自动退出:projectIdList为空 => ${projectIdList}`);
                return false;
            }
...
`;
            for (let tableName of [
                ...
                SINGLE_T_R_ALARM_CONFIG,
                SINGLE_T_R_USER,
                SINGLE_T_R_PROJECT_MEMBER,
                SINGLE_T_R_NEW_USER_SUMMARY,
                SINGLE_T_R_ALARM_LOG,
            ]) {
                let content = generate(tableName)[0];
                commonSqlContent = `${commonSqlContent}\n${content}`;
            }
            let finishAtMoment = moment(finishAtYm, config_1.COMMAND_ARGUMENT_BY_MONTH);
            let sqlContent = '-- --------分表部分---------';
            for (let projectId of projectIdList) {
                for (let currentAtMoment = moment(startAtYm, config_1.COMMAND_ARGUMENT_BY_MONTH); currentAtMoment.isBefore(finishAtMoment.clone().add(1, 'months')); currentAtMoment = currentAtMoment.clone().add(1, 'months')) {
    ...
    handle(args, options) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.execute(args, options).catch(e => {
