        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                for (let [projectId, dbRecordMap] of this.projectMap) {
                    ...
                    let existUcidSet = yield this.userFirstLoginAtService.filterExistUcidSetInDb(projectId, ucidList);
                    for (let [ucid, dbRecord] of dbRecordMap) {
                        ...
                        if (existUcidSet.has(ucid) === false) {
                            ...
                            let oldRecordList = yield this.userFirstLoginAtService.getOldRecordList(projectId, ucid);
                            ...
                            if (id > 0) {
                                if (oldFirstVisitAt > 0 && oldFirstVisitAt > firstVisitAt) {
                                    let affectRows = yield this.userFirstLoginAtService.updateUserFirstLoginAt(id, data, projectId);
                                    isSuccess = affectRows > 0;
                                }
                            }
                            else {
                                data['create_time'] = updateAt;
                                let insertResult = yield this.userFirstLoginAtService.insertUserFirstLoginAt(data, projectId);
