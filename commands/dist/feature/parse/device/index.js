            ...
            for (let [projectId, visitAtMap] of this.projectMap) {
                for (let [visitAtMonth, deviceMap] of visitAtMap) {
                    for (let [uuid, deviceRecord] of deviceMap) {
                        ...
                        let oldListRest = yield this.deviceService.getOldList(projectId, visitAtMonth, uuid);
                        let id = _.get(oldListRest, [0, 'id'], 0);
                        let updateAt = moment().unix();
                        let isSuccess = false;
                        if (id > 0) {
                            sqlParams['update_time'] = updateAt;
                            let affectRows = yield this.deviceService.updataDevice(sqlParams, projectId, id);
                            isSuccess = affectRows > 0;
                        }
                        else {
                            sqlParams['create_time'] = updateAt;
                            sqlParams['update_time'] = updateAt;
                            const insertId = yield this.deviceService.createDevice(sqlParams, projectId);
