...
class MenuClick extends parseBase_1.default {
        ...
        for (let [projectId, countAtMap] of this.projectMap) {
            for (let [countAtTime, codeMap] of countAtMap) {
                for (let [code, recordPackage] of codeMap) {
                    ...
                    let oldRecordList = await this.behaviorDistributionService.replaceRecord(projectId, code, countAtTime, config_1.UNIT.HOUR);
    ...
    async updateCityDistribution(id, data, projectId, createTimeInDb, cityDistribute, cityDistributeIdInDb) {
        ...
        let affectRows = await this.behaviorDistributionService.updateBehavior(id, data);
        return affectRows > 0;
    }
    async insertCityDistributionRecord(cityDistribute, data, projectId, updateAt) {
        ...
        let insertResult = await this.behaviorDistributionService.insertDuration(data);
