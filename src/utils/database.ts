 ...
 */
function getTableNameListInRange(
  projectId,
  startAt,
  finishAt,
  getTableFunc,
  timeSplitUnit = UNIT.MONTH,
) {
...
function computePercent(dividend, divisor, showAsPercent = true) {
  ...
  let resultInt = (dividend / divisor) * 10000;
...
function paddingTimeList(
  ...
  for (
    let timeAt = startMoment.unix();
    timeAt <= endMoment.unix();
    timeAt += timeStep
  ) {
...
function getDatabaseTimeList(startAt, endAt, type) {
  ...
  switch (type) {
    case UNIT.MINUTE:
      dateBaseFormat = DATABASE_BY_MINUTE;
      timeStep = 60;
      break;
    case UNIT.HOUR:
      dateBaseFormat = DATABASE_BY_HOUR;
      timeStep = 3600;
      break;
    case UNIT.DAY:
      dateBaseFormat = DATABASE_BY_DAY;
      timeStep = 86400;
      break;
    default:
      return [];
